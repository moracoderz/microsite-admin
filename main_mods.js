...
const agregarCliente = () => {    
    ...
    const obtenerPrecioPlan = (plan) => {
    planes.map(n => {
        if (n.idPlan === plan) { 
        return (n.precio) }  else { 
            null } 
        });
}

const obtenerPrecioSubPlan = (subPlan) => {
    planes.forEach(n => {
        
        n.subPlanes.map(subPlanes => {
            subPlan === subPlanes.idSub ? console.log(subPlanes.precio) : null })
    });
}

const obtenerTotal = (obtenerPrecioPlan,obtenerPrecioSubPlan) => {
    console.log(obtenerPrecioPlan);
}



    //total = getPlanSubPlanPrice(plan, subPlan);
    obtenerTotal(obtenerPrecioPlan(plan),obtenerPrecioPlan(subPlan));

    li.textContent = `Nombre: ${nombreCli} \nPlan: ${plan} \nSub Plan: ${subPlan} \nTotal: ${total}`;
    
    clientes.push(new Cliente(nombreCli, plan, subPlan, total));
    console.log(new Cliente(nombreCli, plan, subPlan, total));
